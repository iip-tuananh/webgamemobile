...
class ClientRegisterController extends Controller
{
    ...
    public function loginClientSubmit(Request $request)
    {
        ...
        if ($validate->fails()) {
            ...
            return $this->responseErrors("Thông tin đăng nhập chưa đủ!", $validate->errors());
        }
        ...
        if (Auth::attempt($loginConditions, $remember)) {
    ...
    public function registerClientSubmit(Request $request) {
		...
		try {
			$object = new User();
			$object->name = $request->name ?? $request->account_name;
