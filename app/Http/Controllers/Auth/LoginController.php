...
class LoginController extends Controller
{
    ...
    public function login (Request $request)
    {
        ...
        if (Auth::attempt(['account_name' => $request->account_name, 'password' => $request->password, 'status' => 1, 'type' => 1], $remember)) {
    ...
    public function logout() {
        if (Auth::user()->type == User::SUPER_ADMIN) {
            $url = 'login';
        } else {
            $url = 'front.login-client';
        }
        Auth::logout();
        $message = array(
            "logout" => "logout"
        );
        return redirect()->route($url)->with($message);
    }
}
